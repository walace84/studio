...
@section('content')
    ...
    <a href="{{route('listtwo')}}" class="btn"><i class="small material-icons">add_circle</i></a>
    <a href="#" class="btn">{{date('d/m/Y')}}</a>
    
    <div class="col-md-12"> 
    <form action="{{route('shedulestwo')}}" method="POST">
      {{ csrf_field() }}
    ...
    <table>
      ...
      @foreach($data as $client)
        <tr style="color:{{$client->color}}">
          ...
          <td>
            <a href="{{route('edit2', $client->id)}}"><i class="small material-icons">edit</i></a>
          </td>
          <td>
            <a href="{{route('deleta2', $client->id)}}"><i class="small material-icons">delete_forever</i></a>
          </td>
        </tr>
      @endforeach
      </tbody>
